	...
	if ($upfil){
        ...
        <br /><br /><br />
		  ...
		  while($i<$fin){
					...
					// Inserta PROGRAMAS
					$sql8="SELECT count(cod_programa) as cprg FROM programas WHERE cod_programa='".$datos[23]."'";
					//SELECT cod_programa, nom_programa, version, cod_sector FROM programa
					//echo $sql8;
					$data8=$conexion->SelectDato($sql8);
					if($data8[0][0]==0){
						$sql9= "insert into programas (cod_programa, nom_programa, cod_nivel) values ('".$datos[23]."',
						'".$datos[25]."')";
				//echo $sql9;
						$conexion->insertar($sql9);
					}
					
					// Insertar Programa Centro
					//SELECT cod_programa, cod_centro FROM programa_centro
					$sql10="SELECT count(cod_programa) as cpc FROM programa_centro WHERE cod_programa='".$datos[24]."' and cod_centro='".$datos[2]."'";
					//echo $sql10;
					$data10=$conexion->ejecutarConsulta($sql10,0);
					if($data10[0]["cpc"]==0){
						$sql11= "insert into programa_centro (cod_programa, cod_centro) values ('".$datos[24]."','".$datos[2]."')";
				//echo $sql11;
						$conexion->ejecutarConsulta($sql11,1);
					}
					
					// Insertar Programa Titulada
				$sql14="SELECT count(cod_titulacion) as cnfch FROM titulacion WHERE cod_titulacion='".$datos[4]."'";
				//echo $sql14;
				$data14=$conexion->insertar($sql14,0);
				if($data14[0]["cnfch"]>0){
					//SELECT cod_programa, NoFicha, fecha_inicio, fecha_fin FROM progra_titulada
					$sql12="SELECT count(cod_programa) as cpct FROM programas WHERE cod_programa='".$datos[24]."' and cod_titulacion='".$datos[4]."'";
					//echo $sql12;
					$data12=$conexion->insertar($sql12,0);
					if($data12[0]["cpct"]==0){
						$sql13= "insert into titulacion (cod_titulacion, cod_programa, fecha_inicio, fecha_fin) values ('".$datos[24]."','".$datos[4]."','".$datos[12]."','".$datos[13]."')";
						//echo $sql13;
						$conexion->insertar($sql13,1);
					}
		  		}
